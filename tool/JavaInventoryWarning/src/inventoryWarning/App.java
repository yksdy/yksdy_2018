...
public class App {
	...
	void readKJ_Data(Sheet sheet) {
		...
		for (int row = 1; row < rows; row++) {
			...
			for (int col = 0; col < cols; col++) {
				System.out.printf(row + "     ", sheet.getCell(col, row).getContents());
			}
			System.out.println();
		}
	}
	...
	public void writeData(String fileName) throws Exception {
		...
		sheet.addCell(new Label(0, row, "KJ-MaterialId"));
		...
		sheet.addCell(new Label(4, row, "BH-Quantity"));
		...
		for (String key : error_data) {
			...
			if (bhD == null) {
				...
				row++;
				 ...
				 sheet.addCell(new Label(4, row, String.valueOf(bhD.getQuantity())));
